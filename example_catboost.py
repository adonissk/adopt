...
def main():
    """Example usage of the Bayesian Hyperparameter Optimizer with CatBoost."""
    ...
    # Initialize optimizer with BOTH iteration-level and trial-level early stopping
    
    # Trial-level early stopping config (between trials)
    ...
    optimizer = BayesianHyperparameterOptimizer(
        model_type="catboost",
        experiment_name="catboost_regression_example",
        ...
        # Wandb logging
        use_wandb=True,
        wandb_project="catboost-bayesian-optimization",
        wandb_entity=None  # Use default entity
    )
    
    print(f"Experiment ID: {optimizer.experiment_id}")
    
    # Setup experiment with default CatBoost search space
    print("Setting up experiment...")
    optimizer.setup_experiment(df)
    
    # Run optimization
    print("Starting optimization...")
    results = optimizer.optimize(total_trials=10)  # Use 10 trials for quick example
